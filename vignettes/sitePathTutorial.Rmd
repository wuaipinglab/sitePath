...
Tree must be rooted before further analysis

```{r root_tree}
tree <- root(tree, "ANK57896")
```

Occasionally you may get S4 phylo4 class from `ggtree` or other packages, use the `@` symbol to get the S3 phylo object from it
...
Most multiple sequence alignment format can be parsed by [seqinr](https://cran.r-project.org/web/packages/seqinr/index.html).

```{r read_alignment, message=FALSE}
library(seqinr)
...
There might be one point where the number of resulting paths has an **unusual drop** and that could be the preferable similarity threshold. For example, here 0.996 is a reasonable choice. You can use the function `sitePath` to get a S3 sitePath class object^[The S3 sitePath object contains the nodes to represent phylogenetic lineages] for downstream analysis.
...
The output of `fixtionSites` is an R list. To get the tip names involved in a mutation, you can use index or mutation name to subset the R list
...
```{r plot_fixations, fig.show="hold", fig.width=4}
par(mar = c(1,1,1,1))
...
```{r plot_sites, fig.show="hold", fig.width=4}
par(mar = c(1,1,1,1))
plot(fixations, 139)
plot(fixations, 763)
```
...
```{r find_SNP, fig.show="hold", fig.width=4}
