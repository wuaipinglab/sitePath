...
fixationSites.lineagePath <- function(paths, tolerance = 0.01, minEffectiveSize = NULL,
    ...
    if (!is.numeric(tolerance)) {
        ...
        toleranceDesc <- if (length(tolerance) == 1) {
            toleranceAnc
        } else {
            tolerance[2]
        }
    ...
    if (is.null(minEffectiveSize)) {
        ...
        minDesc <- if (length(minEffectiveSize) == 1) {
            minAnc
        } else {
            minEffectiveSize[2]
        }
    ...
    # named by the site predicted to experience fixation and an attribute names
    # 'site' also stores the site index. The info for a fixation ('mutPath') consists
    # of a list of grouped tips with their AA as the name and 'AA' attribute.
    mutations <- list()
    for (minLen in 2:max(lengths(paths))) {
        # Iterate all lineagePath at the same time
        for (path in unique(ancestralPaths(paths, minLen))) {
            # First, the number of tips should meet the minimum size otherwise we'll go for
            # the next node
            ...
            # The 'excludedTips' are the descendants of the 'divNodes' which are not belong
            # to the 'lineagePath'
            excludedTips <- lapply(intersect(pathBefore, divNodes), FUN = function(node) {
                children <- tree$edge[which(tree$edge[, 1] == node), 2]
                children <- children[which(children > length(tree$tip.label) & !children %in%
                  path)]
                return(.childrenTips(tree, children))
            })
            beforeTips <- setdiff(x = seq_along(tree$tip.label), y = c(afterTips,
                unlist(excludedTips)))
            ...
            for (i in seq_along(reference)) {
                ...
                if (is.na(b) || is.na(a) || a == b) {
                  # The AA should be fixed for both ancestral and descendant group. Plus the fixed
                  # AA can't be same. Otherwise, jump to the next site.
                  next
                }
                site <- as.character(i)
                # Use the site to retrieve possible existing 'sitePath'
                existPath <- mutations[[site]]
                # Get the tips names for two groups from <- tree$tip.label[beforeTips] to <-
                # tree$tip.label[afterTips] Use 'targetIndex' to deciden where to insert the
                # tip/AA names in the 'sitePath' entry.
                if (is.null(existPath)) {
                  # Create an entry for the site if it's not existed
                  targetIndex <- 1
                } else {
                  # Use 'mut' to retrieve possible existing 'sitePath'.  The reason for not using
                  # 'mut' as the name of each mutation is that same mutaiton fixation could
                  # theoretically happen on more than one lineages.  Using 'mut' as the name will
                  # cause the conflict and override some fixation events.
                  mut <- paste(c(b, a), collapse = "")
                  existIndex <- which(vapply(existPath, FUN = function(i) {
                  ...
                  if (length(existIndex) == 0) {
                    ...
                    qualified <- vapply(existIndex, FUN = function(ei) {
                    ...
                    # Extend the 'sitePath' when all existing paths have an 'adding state' of 1L,
                    # being different from the 's'
                    if (all(qualified == 1L)) {
                      targetIndex <- length(existPath) + 1
                    } else if (any(r <- qualified == 2L)) {
                      # Remove existing paths with an 'adding state' of 2L and add the 's'
                      mutations[[site]] <- mutations[[site]][-which(r)]
                      targetIndex <- length(mutations[[site]]) + 1
                    }
                  }
                }
